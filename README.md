[![nuget Generators](https://img.shields.io/nuget/v/Friflo.Vectorization.Generators?label=Generators&logo=nuget&logoColor=white)](https://www.nuget.org/packages/Friflo.Vectorization.Generators)
[![nuget WebGPU](https://img.shields.io/nuget/v/Friflo.Vectorization.WebGPU?label=WebGPU&logo=nuget&logoColor=white)](https://www.nuget.org/packages/Friflo.Vectorization.WebGPU)
...
dotnet add package Friflo.Vectorization.Generators
```

#### Method vectorization

A vectorized method is generated by providing a method without a loop annotated with `[Vectorize]`.  
Parameters annotated with `[Span]` are converted to `Span<>`s in the generated method.
```cs
[Vectorize]
