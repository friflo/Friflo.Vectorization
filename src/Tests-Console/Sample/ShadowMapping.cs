...
public partial class Renderer : IRenderer
{
    ...
    private static Matrix4x4 GetCameraViewProjMatrix(GpuExtent3D size, float now)
    {
        var projectionMatrix = Matrix4x4.CreatePerspectiveFieldOfView(2f * MathF.PI / 5f, size.AspectRatio, 1f, 2000f);
    ...
    public void OnFrame(in RenderTarget target)
    {
        ...
        var cameraViewProj = GetCameraViewProjMatrix(target.TargetSize, time);
