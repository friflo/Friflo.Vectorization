...
public readonly ref partial struct GuiWidget
{
    internal bool Slider(ReadOnlySpan<char> name, ref float value, float min, float max, float width, ReadOnlySpan<char> format, GuiStyle? style, WidgetID id)
    {
        ...
        if (tui != null) {
            tui.DrawSlider(labelText.Span(), pos, totalSize, fillSize, Colors.TextColor, Colors.SliderState(widgetState), Colors.SliderColor, isFocused);
        } else {
        ...
        if (isFocused) {
            ...
            EnsureVisibleInScrollArea(pos, totalSize);
        }
