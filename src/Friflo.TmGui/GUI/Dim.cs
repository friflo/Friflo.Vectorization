...
#region Size
    ...
    [Hide] public static Dim   Size(float width, float height) => new(width,   Sizing.Exact,   height, Sizing.Exact);
    
    /// <summary>Sizes both axes to explicit vector pixel values.</summary>
    [Hide] public static Dim   Size(Vector2 size)              => new(size.X,  Sizing.Exact,   size.Y, Sizing.Exact);
    
    /// <summary>Sizes the width to an explicit value and height to inner content bounds.</summary>
    [Hide] public static Dim   Size(float width, Fit Content)  => new(width,   Sizing.Exact,   0f,     Sizing.Content);

    /// <summary>Sizes the width to inner content bounds and height to an explicit value.</summary>
    [Hide] public static Dim   Size(Fit Content, float height) => new(0f,      Sizing.Content, height, Sizing.Exact);
#endregion  


#region Fill
    /// <summary>Fills remaining parent width with a right margin and sets explicit height.</summary>
    [Hide] public static Dim   Fill_X(float distRight, float height)   => new(distRight,   Sizing.Fill,    height,     Sizing.Exact);

    /// <summary>Fills remaining parent width with a right margin and sizes height to content.</summary>
    [Hide] public static Dim   Fill_X(float distRight, Fit Content)    => new(distRight,   Sizing.Fill,    0f,         Sizing.Content);
    
    /// <summary>Sizes width to an explicit value and fills remaining parent height with a bottom margin.</summary>
    [Hide] public static Dim   Fill_Y(float width, float distBottom)   => new(width,       Sizing.Exact,   distBottom, Sizing.Fill);

    /// <summary>Sizes width to inner content bounds and fills remaining parent height with a bottom margin.</summary>
    [Hide] public static Dim   Fill_Y(Fit Content, float distBottom)   => new(0f,          Sizing.Content, distBottom, Sizing.Fill);
    
    /// <summary>Fills all available parent space on both axes.</summary>
    [Hide] public static Dim   Fill()                                  => new(0f,          Sizing.Fill,    0f,         Sizing.Fill);
    
    /// <summary>Fills remaining parent space on both axes with a right and bottom margin.</summary>
    [Hide] public static Dim   Fill(float distRight, float distBottom) => new(distRight,   Sizing.Fill,    distBottom, Sizing.Fill);
#endregion


#region Content
    /// <summary>Sizes both axes according to inner content bounds.</summary>
    [Hide] public static Dim   Content() => default;
#endregion
}