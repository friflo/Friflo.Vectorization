...
public static class WgpuPattern
{
        ...
        in InOutBuffer<float>   output)
    {
        ...
        recorder.InitKernel(MultiplyAdd_GPU_KernelId, "MultiplyAdd"u8);
        ...
        if (!bindGroupCache.bufferGroup.TryGetValue(key, out var bufferGroup)) {
            ...
            bufferGroup = recorder.CreateBindGroup(pipelineCache.layouts[0], "MultiplyAdd_buffers"u8);
        ...
        pass.SetBindGroupUniform(1, 0, ref bindGroupCache.uniformGroup, uniforms, pipelineCache, "MultiplyAdd_uniforms"u8);
