...
namespace Kernel.Generators
{
        ...
        in float           max)
    {
        ...
        ref var effect = ref device.GetComputeEffect(_Kernel_Clamp_GPU_KernelId, _Kernel_Clamp_GPU_WgslHash);
        if (!effect.IsCreated) {
            effect = ref _Kernel_Clamp_GPU_CreateEffect(device);
        }
        ...
        if (!bufferGroup.IsCreated) {
            ...
            device.UpdateComputeCache(_Kernel_Clamp_GPU_KernelId, bufferGroup, buffers.hash);
        }
    ...
    private static ref WgpuComputeEffect _Kernel_Clamp_GPU_CreateEffect(WgpuDevice device)
    {
        // @group(0)
        ...
        return ref device.CreateComputeEffect(_Kernel_Clamp_GPU_KernelId, _Kernel_Clamp_GPU_WgslHash, pipeline, default, bufferLayout, uniformLayout);
    }
