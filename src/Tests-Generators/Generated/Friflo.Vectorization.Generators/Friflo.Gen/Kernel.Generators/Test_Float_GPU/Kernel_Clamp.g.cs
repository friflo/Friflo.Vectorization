...
namespace Kernel.Generators
{
        ...
        in float           max)
    {
        ...
        if (!bindGroupCache.bufferGroup.TryGetValue(key, out var bufferGroup)) {
            ...
            bufferGroup = recorder.CreateBindGroupNew(pipelineCache.bufferLayout, entries, "Kernel_Clamp_buffers"u8);
        ...
        pass.SetUniformBindGroup(1, pipelineCache, ref bindGroupCache.uniformGroup, uniforms, "Kernel_Clamp_uniforms"u8);
    ...
    private static ref ComputeCache _Kernel_Clamp_GPU_CreateComputeCache(WgpuDevice device)
    {
        // @group(0)
        ...
        if (!bufferLayout.IsCreated) {
            ...
            buffers[1] = WgpuLayoutEntry.ReadOnlyStorage (1); // var<storage, read      >  min_arr: array<f32>;
            bufferLayout = device.CreateBindGroupLayout(buffers, ShaderStage.Compute, false, _Kernel_Clamp_GPU_BufferLayoutKey, "Kernel_Clamp_buffers"u8);
        }
        // @group(1)
        ...
        if (!uniformLayout.IsCreated) {
            ...
            uniformLayout = device.CreateBindGroupLayout(uniform, ShaderStage.Compute, true, _Kernel_Clamp_GPU_UniformLayoutKey, "Kernel_Clamp_uniforms"u8);
        }
