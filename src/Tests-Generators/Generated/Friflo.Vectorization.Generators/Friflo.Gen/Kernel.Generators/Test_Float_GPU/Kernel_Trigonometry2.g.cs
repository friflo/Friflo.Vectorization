...
namespace Kernel.Generators
{
        ...
        in InOutBuffer<float> position)
    {
        ...
        ref readonly var pipelineCache = ref device.GetPipelineCache(_Kernel_Trigonometry2_GPU_KernelId, _Kernel_Trigonometry2_GPU_WgslHash);
        if (!pipelineCache.IsCreated) {
            pipelineCache = ref _Kernel_Trigonometry2_GPU_CreateComputeCache(device);
        }
        pass.SetPipeline(pipelineCache.computePipeline);

        var bindGroupCache = (_Kernel_Trigonometry2_GPU_Cache)pipelineCache.bindGroupCache;
        ...
        if (!bindGroupCache.bufferGroup.TryGetValue(key, out var bufferGroup)) {
            ...
            bufferGroup = recorder.CreateBindGroup(pipelineCache.bufferLayout, entries, "Kernel_Trigonometry2_buffers"u8);
        ...
        pass.SetUniformBindGroup(1, ref bindGroupCache.uniformGroup, pipelineCache, uniforms, "Kernel_Trigonometry2_uniforms"u8);
        
        pass.DispatchWorkgroups((buffers.length + 63) / 64, 1, 1);
    }

    private sealed class _Kernel_Trigonometry2_GPU_Cache : BindGroupCache
    {
    ...
    private static ref readonly ComputeCache _Kernel_Trigonometry2_GPU_CreateComputeCache(WgpuDevice device)
    {
        // @group(0)
        ...
        if (!bufferLayout.IsCreated) {
            ...
            bufferLayout = device.CreateBindGroupLayout(buffers, ShaderStage.Compute, _Kernel_Trigonometry2_GPU_BufferLayoutKey, "Kernel_Trigonometry2_buffers"u8);
        }
        // @group(1)
        ...
        if (!uniformLayout.IsCreated) {
            ...
            uniformLayout = device.CreateBindGroupLayout(uniform, ShaderStage.Compute, _Kernel_Trigonometry2_GPU_UniformLayoutKey, "Kernel_Trigonometry2_uniforms"u8);
        }
