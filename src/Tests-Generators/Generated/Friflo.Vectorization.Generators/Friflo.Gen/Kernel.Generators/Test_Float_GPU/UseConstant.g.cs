...
namespace Kernel.Generators
{
        ...
        in InOutBuffer<float> position)
    {
        ...
        ref var effect = ref device.GetComputeEffect(_UseConstant_GPU_KernelId, _UseConstant_GPU_WgslHash);
        if (!effect.IsCreated) {
        ...
        if (!bufferGroup.IsCreated) {
            ...
            device.UpdateComputeCache(_UseConstant_GPU_KernelId, bufferGroup, buffers.hash);
        }
    ...
    private static ref WgpuComputeEffect _UseConstant_GPU_CreateEffect(WgpuDevice device)
    {
        // @group(0)
        ...
        if (!bufferLayout.IsCreated) {
            ...
            bufferLayout = device.CreateBindGroupLayout(buffers, ShaderStage.Compute, false, _UseConstant_GPU_BufferLayoutKey, "UseConstant_buffers"u8);
        }
        // @group(1)
        ...
        if (!uniformLayout.IsCreated) {
            ...
            uniformLayout = device.CreateBindGroupLayout(uniform, ShaderStage.Compute, true, _UseConstant_GPU_UniformLayoutKey, "UseConstant_uniforms"u8);
        }
        ...
        return ref device.CreateComputeEffect(_UseConstant_GPU_KernelId, _UseConstant_GPU_WgslHash, pipeline, default, bufferLayout, uniformLayout);
    }
