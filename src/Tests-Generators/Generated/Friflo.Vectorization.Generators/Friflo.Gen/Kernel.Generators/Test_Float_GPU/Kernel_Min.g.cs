...
namespace Kernel.Generators
{
        ...
        in InBuffer<float> velocity_)
    {
        ...
        recorder.Init(_Kernel_Min_GPU_EffectSlot);
        ...
        var encoder = recorder.GetEncoder("Kernel_Min"u8);
        using (var pass = encoder.BeginComputePass("Kernel_Min"u8))
        {
            ref var effect = ref device.GetEffect(_Kernel_Min_GPU_EffectSlot); // simple GpuEffect[] array lookup
            if (!effect.IsCreated) {
            ...
            if (!bufferGroup.IsCreated) {
                ...
                bufferGroup = recorder.CreateBindGroup(effect.bufferLayout, entries, "Kernel_Min_buffers"u8);
            ...
            var uniformGroup = recorder.CreateBindGroup(effect.uniformLayout, entry, "Kernel_Min_uniforms"u8);
        ...
        recorder.Finish(encoder, "Kernel_Min"u8);
    ...
    private static ref WgpuEffect _Kernel_Min_GPU_CreateEffect(WgpuDevice device)
    {
        ...
        return ref device.CreateEffect(_Kernel_Min_GPU_EffectSlot, pipeline, bufferLayout, uniformLayout);
    }
