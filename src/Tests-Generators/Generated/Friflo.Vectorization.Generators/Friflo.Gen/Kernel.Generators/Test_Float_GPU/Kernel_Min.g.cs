...
namespace Kernel.Generators
{
        ...
        in InBuffer   <float> velocity)
    {
        ...
        if (!bindGroupCache.bufferGroup.TryGetValue(key, out var bufferGroup)) {
            ...
            bufferGroup = recorder.CreateBindGroupNew(pipelineCache.bufferLayout, entries, "Kernel_Min_buffers"u8);
        ...
        pass.SetUniformBindGroup(1, pipelineCache, ref bindGroupCache.uniformGroup, uniforms, "Kernel_Min_uniforms"u8);
    ...
    private static ref ComputeCache _Kernel_Min_GPU_CreateComputeCache(WgpuDevice device)
    {
        ...
        if (!bufferLayout.IsCreated) {
            ...
            bufferLayout = device.CreateBindGroupLayout(buffers, ShaderStage.Compute, false, _Kernel_Min_GPU_BufferLayoutKey, "Kernel_Min_buffers"u8);
        }
        // @group(1)
        ...
        if (!uniformLayout.IsCreated) {
            ...
            uniformLayout = device.CreateBindGroupLayout(uniform, ShaderStage.Compute, true, _Kernel_Min_GPU_UniformLayoutKey, "Kernel_Min_uniforms"u8);
        }
