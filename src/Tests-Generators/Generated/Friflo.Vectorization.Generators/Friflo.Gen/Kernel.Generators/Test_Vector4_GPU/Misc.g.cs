...
namespace Kernel.Generators
{
        ...
        in Vector4         max)
    {
        ...
        ref readonly var pipelineCache = ref device.GetPipelineCache(_Misc_GPU_KernelId, _Misc_GPU_WgslHash);
        if (!pipelineCache.IsCreated) {
            pipelineCache = ref _Misc_GPU_CreateComputeCache(device);
        }
        pass.SetPipeline(pipelineCache.computePipeline);

        var bindGroupCache = (_Misc_GPU_Cache)pipelineCache.bindGroupCache;
        ...
        if (!bindGroupCache.bufferGroup.TryGetValue(key, out var bufferGroup)) {
            ...
            bufferGroup = recorder.CreateBindGroup(pipelineCache.bufferLayout, entries, "Misc_buffers"u8);
        ...
        pass.SetUniformBindGroup(1, ref bindGroupCache.uniformGroup, pipelineCache, uniforms, "Misc_uniforms"u8);
        
        pass.DispatchWorkgroups((buffers.length + 63) / 64, 1, 1);
    }

    private sealed class _Misc_GPU_Cache : BindGroupCache
    {
    ...
    private static ref readonly ComputeCache _Misc_GPU_CreateComputeCache(WgpuDevice device)
    {
        // @group(0)
        ...
        if (!bufferLayout.IsCreated) {
            ...
            bufferLayout = device.CreateBindGroupLayout(buffers, ShaderStage.Compute, _Misc_GPU_BufferLayoutKey, "Misc_buffers"u8);
        }
        // @group(1)
        ...
        if (!uniformLayout.IsCreated) {
            ...
            uniformLayout = device.CreateBindGroupLayout(uniform, ShaderStage.Compute, _Misc_GPU_UniformLayoutKey, "Misc_uniforms"u8);
        }
