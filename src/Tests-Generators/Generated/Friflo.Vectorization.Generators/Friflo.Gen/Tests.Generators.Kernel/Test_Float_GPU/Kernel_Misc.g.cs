...
namespace Tests.Generators.Kernel
{
    public partial class Test_Float_GPU
    {
        ...
        public static GpuBuffer<float> Kernel_MiscKernel(
            ...
            ComputeMode mode = ComputeMode.Device)
        {
            ...
            if (buffers.ComputeGPU) {
                return _Kernel_Misc_GPU(buffers, position, velocity, value);
            }
    ...
    [SkipLocalsInit]
    private static GpuBuffer<float> _Kernel_Misc_GPU(
        ...
        in float           value)
    {
        ...
        var encoder = task.GetEncoder("Kernel_Misc"u8);
        using (var pass = encoder.BeginComputePass("Kernel_Misc"u8))
        {
            var effect = device.GetEffect(_Kernel_Misc_GPU_EffectSlot); // simple GpuEffect[] array lookup
            if (!effect.IsCreated) {
                effect = _Kernel_Misc_GPU_CreateEffect(device);
            }
            ...
            if (!bufferGroup.IsCreated) {
                ...
                bufferGroup = task.CreateBindGroup(effect.bufferLayout, entries, "Kernel_Misc_buffers"u8);
        ...
        task.Finish(encoder, "Kernel_Misc"u8);
    ...
    private static WgpuEffect _Kernel_Misc_GPU_CreateEffect(WgpuDevice device)
    {
        // @group(0)
        ...
        if (!uniformLayout.IsCreated) {
            ...
            uniform[0]    = WgpuLayoutEntry.Uniform<_Kernel_Misc_GPU_Uniforms> (0); // var<uniform>              uniforms
        ...
        return device.CreateEffect(_Kernel_Misc_GPU_EffectSlot, pipeline, bufferLayout, uniformLayout);
    }
