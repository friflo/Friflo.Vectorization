...
namespace Kernel.Lab
{
        ...
        in InBuffer   <float> input)
    {
        ...
        recorder.InitKernel(_Assign_GPU_KernelId, "Assign"u8);
        ...
        {
            recorder.BindGroupEntryBuffer(0, output.Buffer);
            recorder.BindGroupEntryBuffer(1, input.Buffer);
            bufferGroup = recorder.CreateBindGroup(pipelineCache.bufferLayout, "Assign_buffers"u8);
        ...
        pass.SetBindGroupUniform(1, 0, ref bindGroupCache.uniformGroup, uniforms, pipelineCache, "Assign_uniforms"u8);
    ...
    private static ref readonly ComputeCache _Assign_GPU_CreateComputeCache(WgpuDevice device)
    {
        // @group(0)
        var bufferLayout = device.GetBindGroupLayout(_Assign_GPU_BufferLayoutKey);
        if (!bufferLayout.IsCreated)
        {
            device.BindGroupLayoutBuffer(0, BufferBindingType.Storage);         // var<storage, read_write>  output_arr: array<f32>;
            device.BindGroupLayoutBuffer(1, BufferBindingType.ReadOnlyStorage); // var<storage, read      >  input_arr: array<f32>;
            bufferLayout = device.CreateBindGroupLayout(ShaderStage.Compute, _Assign_GPU_BufferLayoutKey, "Assign_buffers"u8);
        }
        // @group(1)
        ...
        if (!uniformLayout.IsCreated) {
            ...
            uniformLayout = device.CreateBindGroupLayout(ShaderStage.Compute, _Assign_GPU_UniformLayoutKey, "Assign_uniforms"u8);
        }
        using var shaderModule  = device.CreateShaderModule(_Assign_GPU_Shader(), "Assign"u8);
