...
public static class Tests_GenerateTypes
{
    ...
    public static void Tests_WGSL_GenerateAllTypes()
    {
        ...
        var mappings = WgslTypeMappings.LoadTypeMapping($"{projectDir}/wgsl-type-map.json", out var error);
        if (error != null) {
            Assert.Fail(error);
        }
        ...
        Assert.That(mappings, Has.Member(new WgslType2CSharpType(CsTypeCode.vec2u,   "CustomTypes",         "Vector2<uint>")));
